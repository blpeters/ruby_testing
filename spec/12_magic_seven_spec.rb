...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 11 and the random number is 4' do
      subject(:game_with_4) { described_class.new(4)}
      it 'returns 7' do
        previous_step = 11
        result = game_with_4.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 12' do
      subject(:game_with_12) { described_class.new(12) }
    ...
    context 'when the random number is 0' do
      subject(:game_with_0) { described_class.new(0) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_with_0.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 1' do
      subject(:game_with_1) { described_class.new(1) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_with_1.play
        expect(result).to eq(7)
      end
    end
  end
