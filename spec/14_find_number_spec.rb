...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_win) { described_class.new(0, 9, number_guessing, 4)}
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('number_guessing', value: 4) }
      subject(:game_win) { described_class.new(0, 9, number_guessing, 5)}

      it 'is not game over' do
        expect(game_win).not_to be_game_over
      end
    end
  end
